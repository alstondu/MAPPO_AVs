...
def main():
    ...
    file_configs = [
        ...
        {'path': 'idm.json', 'step_range': range(0, 5000, 5000), 'label': 'IDM'}
    ]
    ...
    # Create table data
    table_data = [["Algorithm", "Step Range", "Safety Rate", "Success Rate", "Efficiency Rate", "Avg. Episode Reward"]]
    for label, data in metrics_data.items():
        for row in data:
            start_step, safety, success_rate, efficiency, avg_episode_reward = row
            end_step = start_step + 50000
            table_data.append([
                label,
                f"{start_step:d}-{end_step:d}",
                f"{safety:.2f}",
                f"{success_rate:.2f}",
                f"{efficiency:.2f}",
                f"{avg_episode_reward:.2f}"
            ])
