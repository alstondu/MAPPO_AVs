...
class EnvRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                steps_completed = total_num_steps
                total_steps = self.num_env_steps
                steps_remaining = total_steps - steps_completed
                time_remaining = elapsed_time / steps_completed * steps_remaining
                
                print(f"\nScenario {self.all_args.scenario_name} Algo {self.algorithm_name} Exp {self.experiment_name} updates {episode}/{episodes} episodes, total num timesteps {total_num_steps}/{self.num_env_steps}, FPS {int(total_num_steps / elapsed_time)}, Estimated time remaining: {time.strftime('%H:%M:%S', time.gmtime(time_remaining))}")

                train_infos["average_episode_rewards"] = np.mean(self.buffer.rewards) * self.episode_length
                print("average episode rewards is {}".format(train_infos["average_episode_rewards"]))
                self.log_train(train_infos, total_num_steps)
                # self.log_env(env_infos, total_num_steps)
                self.total_reward.append(train_infos["average_episode_rewards"])
    ...
    def insert(self, data):
        ...
        # print(dones.shape, rnn_states.shape)
